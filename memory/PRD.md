...
### Revision-Aware Scoring + Baseline Lift (Completed - Feb 2026)
User: "Coach Max is too tough. Raise the baseline. Reward revisions that address prior growth areas with a bigger jump — students should feel their work moves the needle."
- [x] **Baseline lifted**: `INVESTOR_SCORE_INSTRUCTION` rewritten so the default landing zone for a solid first-time attempt is now **65-79 (Traction Mode)** instead of the old 50-75. Sub-50 explicitly reserved for essentially-blank or off-topic work. Tier thresholds (Bronze 50 / Silver 70 / Gold 85) unchanged so the badge system stays intact.
- [x] **Revision bonus rule**: new "REVISION BONUS" block in the prompt — if the AI sees a "PRIOR ATTEMPTS ON THIS EXACT MILESTONE" section, it must award +10 to +15 points over the previous score when the revision substantively addresses earlier growth areas, +0 to +5 for cosmetic-only revisions, and never lower than prior unless the new version is objectively weaker.
...
### Venture Path — 14 Modules in Curriculum Order (Completed - Feb 2026)
User: "Modules are in completion order not numerical order. Only some modules show — need all 14. Locked modules should still appear."
- [x] **Bug #1 (ordering)**: The badges grid was already numerical, but the score-trend chart was sorted by `submitted_at` chronological. If a student submitted Week 5 before Week 3, the chart showed 5 → 3. Fixed by sorting `trend` by `week` ASC after building it. Also added explicit `sorted(VENTURE_PATH_MODULES, key=lambda x: x["module"])` in the module loop as belt-and-braces.
- [x] **Bug #2 (missing modules)**: `VENTURE_PATH_MODULES` had only 6 entries — extended to **14** covering the full course. Added 8 new named milestones (Traction Engine, Financial Foundations, Legal & Ops, Team Builder, Story Craft, Investor Radar, Fundraise Fluent, Demo Day Ready) with distinct icons. Frontend ICONS map extended with 8 new lucide components (TrendingUp, DollarSign, Shield, Users, Mic, Target, Briefcase, Star).
- [x] **Bonus — real curriculum names**: `_compute_venture_path_for_student` now walks the student's cohort assignments and OVERRIDES the default module name with the actual milestone title where a milestone exists for that week. Locked/never-submitted modules still appear with either the curriculum title or the default fallback name.
- [x] **Tests**: 3 new pytest cases in `test_venture_path.py` covering (a) all 14 modules always present, (b) chronologically-out-of-order submissions rendered in curriculum order in both grid and trend, (c) milestone title overrides default name. Full suite **56/56 pass**.
- [x] **Screenshot verified**: 3/14 badges (1 Gold, 1 Silver, 1 Bronze from seeded scores), 11 locked cards render correctly, trend chart reads Week 1 → Week 3 → Week 7 despite being submitted out of order.

### Editable Venture Path Module Names (Completed - Feb 2026)
User: "Please allow for Editable Module Names."
- [x] **Backend**: `platform_settings._id='venture_path_modules'.overrides` (dict keyed by module number as string) stores per-module `{name, tagline, icon}` overrides. New `_get_venture_path_modules_merged()` helper merges defaults + overrides, with empty/whitespace fields falling through to the built-in default so admins can partially override just what they want customised.
- [x] **Endpoints** (super_admin only): `GET /api/admin/venture-path-modules` returns `{modules, defaults, allowed_icons}`; `PUT /api/admin/venture-path-modules` upserts overrides. Name capped at 80 chars, tagline at 160, icon must be in the 14-item whitelist (mirrors the frontend ICONS map). 403 for students/instructors.
- [x] `_compute_venture_path_for_student` now uses the merged list, so admin overrides propagate to every student view and every instructor "view a student" view. Hierarchy is **cohort milestone title > admin override > code default** (curriculum still wins over admin naming, which is the desired behaviour for cohort-specific curricula).
- [x] **Frontend**: new `/admin/venture-path` page (`VenturePathAdmin.js`) — 14 module rows with name/tagline/icon inputs, per-row **Reset** button that restores defaults in-memory, single **Save** button that PUTs the whole list. Added "Venture Path" nav button on `/admin` (`data-testid='venture-path-admin-nav-btn'`). Route guarded — students navigating there redirect to `/dashboard` and the API call is short-circuited to avoid a flash 403 toast.
- [x] **Tests**: 14 new pytest cases in `TestModuleNameOverrides` covering auth, shape, full/partial overrides, validation (invalid module, invalid icon, name truncation), E2E propagation to `/api/student/venture-path`, and the curriculum-title-wins hierarchy. Full suite **45/45 pass** on `test_venture_path.py`. Frontend Playwright confirmed nav btn, page render, 14 cards, edit-save-reload persistence, reset, and student redirect (iteration_61).
